"""
Vendor FRN Tracking Model

Per-FRN manual annotations a vendor maintains on top of the raw USAC data.
These are working fields USAC does not provide — the vendor's own view of where
each funding request stands in their workflow. This is the vendor-side parallel
of ``ConsultantFrnTracking``.

Covers the same working asks as the consultant tracker:
...
One row per (vendor user, FRN). Upserted from the vendor portal, scoped to the
account owner so team seats share one view.

The allowed value lists (WORKING_STATUS_VALUES / PIA_STATUS_VALUES) are imported
from ``consultant_frn_tracking`` so the two trackers stay in sync — do NOT
redefine them here.
"""
...
class VendorFrnTracking(Base):
    """Vendor-maintained working annotations for a single FRN."""

    __tablename__ = "vendor_frn_tracking"
    __table_args__ = (
        UniqueConstraint("vendor_user_id", "frn", name="uq_vendor_frn_tracking_user_frn"),
    )
