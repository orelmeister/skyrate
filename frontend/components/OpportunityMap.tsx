...
function categoryColor(lead: Form470Lead): string {
  const cats = (lead.categories || []).map((c) => (c || "").toLowerCase());
  const hasC1 = cats.some((c) => c.includes("1"));
  const hasC2 = cats.some((c) => c.includes("2"));
  if (hasC1 && hasC2) return "#7c3aed"; // both -> purple
  if (hasC2) return "#059669"; // Category 2 -> green
  if (hasC1) return "#2563eb"; // Category 1 -> blue
  return "#64748b"; // unknown -> slate
}
...
export default function OpportunityMap() {
  ...
  const filtered = useMemo(() => {
    return geoLeads.filter((l) => {
      ...
      if (catFilter) {
        const cats = (l.categories || []).map((c) => (c || "").toLowerCase());
        if (catFilter === "1" && !cats.some((c) => c.includes("1"))) return false;
        if (catFilter === "2" && !cats.some((c) => c.includes("2"))) return false;
      }
      return true;
    });
  }, [geoLeads, stateFilter, catFilter]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-600 ml-auto">
          <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-full inline-block" style={{ background: "#2563eb" }} /> Cat 1</span>
          <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-full inline-block" style={{ background: "#059669" }} /> Cat 2</span>
          <span className="inline-flex items-center gap-1"><span className="w-3 h-3 rounded-full inline-block" style={{ background: "#7c3aed" }} /> Both</span>
        </div>
      </div>
